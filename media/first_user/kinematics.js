...
function initializeElementValues(){
	document.write(canvas1.scrollWidth);
}
...
function renderWorld(){
	ortho_camera1 = new THREE.OrthographicCamera(
		leftlimit,
		(canvas1.scrollWidth/canvas1.scrollHeight)*(toplimit - bottomlimit) + leftlimit,
		toplimit,
		bottomlimit,
		+500,
		-500);
	...
	renderer1.setSize(canvas1.scrollWidth, canvas1.scrollHeight);
	renderer1.render(scene, ortho_camera1);
}
...
function resizeGreenBall(_scaleFactor) {
	if (_scaleFactor != 0){
...
function moveObject(_object, _x, _y, _z){
	_object.translateX(_x);
	_object.translateY(_y);
	_object.translateZ(_z);
}
